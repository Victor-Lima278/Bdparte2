...
public:
    ...
    // NOVA BUSCA AVANÇADA COM FAIXA DE PREÇO
...
public:
    ...
    bool atualizar(int id, wstring nome, wstring cidade, wstring time, bool anime, wstring& erroMsg) { 
        string query = "UPDATE clientes SET nome = '" + w_para_s(nome) + "', cidade = '" + w_para_s(cidade) + "', time_coracao = '" + w_para_s(time) + "', assiste_one_piece = " + (anime ? "1" : "0") + " WHERE id = " + to_string(id); 
        if (mysql_query(conexao, query.c_str()) == 0) return mysql_affected_rows(conexao) > 0; 
        erroMsg = s_para_w(mysql_error(conexao)); return false; 
    }
};
...
public:
    ...
    bool finalizarVenda(int idC, int idV, int idL, int qtd, double total, string met, wstring& erroMsg) { string query = "CALL realizar_venda(" + to_string(idC) + "," + to_string(idV) + "," + to_string(idL) + "," + to_string(qtd) + "," + to_string(total) + ",'" + met + "')"; if (mysql_query(conexao, query.c_str()) == 0) return true; erroMsg = s_para_w(mysql_error(conexao)); return false; }
    ...
    bool atualizarVendedor(int id, wstring nome, wstring& erroMsg) { 
        string query = "UPDATE vendedores SET nome = '" + w_para_s(nome) + "' WHERE id = " + to_string(id); 
        if (mysql_query(conexao, query.c_str()) == 0) return mysql_affected_rows(conexao) > 0; 
        erroMsg = s_para_w(mysql_error(conexao)); return false; 
    }
};
...
// CONTROLES DE BUSCA AVANÇADA (INCLUINDO PREÇOS)
...
HWND hEditVenIdCli, hEditVenIdVend, hEditVenIdLivro, hEditVenQtd, hComboMetodo, hListViewRanking;
...
// ATUALIZAÇÃO DA TABELA DE LIVROS LENDO OS CAMPOS DE PREÇO
void AtualizarTabelaLivros() {
    ListView_DeleteAllItems(hListViewLivros); 
    
    wchar_t t[256], a[256], tp[50], g[50], pMin[20], pMax[20];
    GetWindowTextW(hBuscaTitulo, t, 256);
    GetWindowTextW(hBuscaAutor, a, 256);
    GetWindowTextW(hBuscaPrecoMin, pMin, 20);
    GetWindowTextW(hBuscaPrecoMax, pMax, 20);
    SendMessage(hBuscaTipo, CB_GETLBTEXT, SendMessage(hBuscaTipo, CB_GETCURSEL, 0, 0), (LPARAM)tp);
    SendMessage(hBuscaGenero, CB_GETLBTEXT, SendMessage(hBuscaGenero, CB_GETCURSEL, 0, 0), (LPARAM)g);

    bool fMari = (SendMessage(hFilterMari, BM_GETCHECK, 0, 0) == BST_CHECKED); 
    bool fEst = (SendMessage(hFilterBaixoEstoque, BM_GETCHECK, 0, 0) == BST_CHECKED);
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE: {
            ...
            // --- PAINEL DE BUSCA AVANÇADA COM PREÇOS ---
            ...
            CriarCtrl(tabLivros, L"STATIC", L"Tipo:", 0, 340, 108, 40, 20, NULL, true); hBuscaTipo = CriarCtrl(tabLivros, L"COMBOBOX", L"", CBS_DROPDOWNLIST, 385, 105, 100, 150, NULL, true);
            SendMessage(hBuscaTipo, CB_ADDSTRING, 0, (LPARAM)L"Todos"); SendMessage(hBuscaTipo, CB_ADDSTRING, 0, (LPARAM)L"Livro"); SendMessage(hBuscaTipo, CB_ADDSTRING, 0, (LPARAM)L"Mangá"); SendMessage(hBuscaTipo, CB_SETCURSEL, 0, 0); 

            CriarCtrl(tabLivros, L"STATIC", L"Gênero:", 0, 495, 108, 60, 20, NULL, true); hBuscaGenero = CriarCtrl(tabLivros, L"COMBOBOX", L"", CBS_DROPDOWNLIST, 560, 105, 120, 150, NULL, true);
            const wchar_t* gensBusca[] = { L"Todos", L"Aventura", L"Ação", L"Romance", L"Terror", L"Ficção", L"Comédia" }; 
            for(int i=0; i<7; i++) SendMessage(hBuscaGenero, CB_ADDSTRING, 0, (LPARAM)gensBusca[i]); SendMessage(hBuscaGenero, CB_SETCURSEL, 0, 0); 
            ...
            // ABA 2, 3 e 4
            CriarCtrl(tabClientes, L"STATIC", L"Nome do Cliente:", 0, 30, 60, 150, 20, NULL); hEditCliNome = CriarEdit(tabClientes, L"", 0, 30, 85, 250, 24, NULL); CriarCtrl(tabClientes, L"STATIC", L"Cidade:", 0, 30, 120, 150, 20, NULL); hEditCliCidade = CriarEdit(tabClientes, L"", 0, 30, 145, 250, 24, NULL); CriarCtrl(tabClientes, L"STATIC", L"Time:", 0, 30, 180, 150, 20, NULL); hEditCliTime = CriarEdit(tabClientes, L"", 0, 30, 205, 250, 24, NULL); hCheckCliAnime = CriarCtrl(tabClientes, L"BUTTON", L"Assiste One Piece? (Desconto)", BS_AUTOCHECKBOX, 30, 240, 250, 20, NULL); CriarCtrl(tabClientes, L"BUTTON", L"Cadastrar Novo Cliente", BS_PUSHBUTTON, 30, 280, 250, 35, (HMENU)ID_BTN_CLI_INSERIR); CriarCtrl(tabClientes, L"BUTTON", L"Salvar Alterações (Edição)", BS_PUSHBUTTON, 30, 320, 250, 35, (HMENU)ID_BTN_CLI_ATUALIZAR);hListViewClientes = CreateWindowExW(WS_EX_CLIENTEDGE, WC_LISTVIEWW, L"", WS_CHILD | LVS_REPORT | LVS_SINGLESEL, 320, 60, 670, 180, hwnd, NULL, NULL, NULL); SendMessage(hListViewClientes, WM_SETFONT, (WPARAM)hFont, TRUE); tabClientes.push_back(hListViewClientes); ConfigListView(hListViewClientes, {L"ID", L"Nome do Cliente", L"Cidade", L"Time", L"Fã O.P.?"}, {40, 200, 120, 120, 100}); CriarCtrl(tabClientes, L"STATIC", L"ID do Cliente:", 0, 320, 255, 100, 20, NULL); hEditCliBuscaId = CriarEdit(tabClientes, L"", ES_NUMBER, 430, 252, 60, 24, NULL); CriarCtrl(tabClientes, L"BUTTON", L"Ver Dados Cadastrais & Histórico", BS_PUSHBUTTON, 500, 250, 250, 28, (HMENU)ID_BTN_CLI_BUSCAR); CriarCtrl(tabClientes, L"STATIC", L"Histórico de Compras Realizadas:", 0, 320, 290, 300, 20, NULL); hListViewHistorico = CreateWindowExW(WS_EX_CLIENTEDGE, WC_LISTVIEWW, L"", WS_CHILD | LVS_REPORT | LVS_SINGLESEL, 320, 315, 670, 145, hwnd, NULL, NULL, NULL); SendMessage(hListViewHistorico, WM_SETFONT, (WPARAM)hFont, TRUE); tabClientes.push_back(hListViewHistorico); ConfigListView(hListViewHistorico, {L"Venda ID", L"Livro Comprado", L"Qtd", L"Valor Pago (R$)", L"Forma Pgto"}, {80, 250, 50, 120, 120});
            CriarCtrl(tabVendedores, L"STATIC", L"Nome do Vendedor:", 0, 30, 60, 150, 20, NULL); hEditNomeVendedor = CriarEdit(tabVendedores, L"", 0, 30, 85, 250, 24, NULL); CriarCtrl(tabVendedores, L"BUTTON", L"Registrar Vendedor", BS_PUSHBUTTON, 30, 120, 250, 35, (HMENU)ID_BTN_VEND_INSERIR);    CriarCtrl(tabVendedores, L"STATIC", L"ID (Para Editar):", 0, 30, 165, 120, 20, NULL); hEditVendId = CriarEdit(tabVendedores, L"", ES_NUMBER, 30, 190, 60, 24, NULL); CriarCtrl(tabVendedores, L"BUTTON", L"Atualizar Vendedor", BS_PUSHBUTTON, 100, 185, 180, 35, (HMENU)ID_BTN_VEND_ATUALIZAR); hListViewVendedores = CreateWindowExW(WS_EX_CLIENTEDGE, WC_LISTVIEWW, L"", WS_CHILD | LVS_REPORT | LVS_SINGLESEL, 320, 60, 670, 400, hwnd, NULL, NULL, NULL); SendMessage(hListViewVendedores, WM_SETFONT, (WPARAM)hFont, TRUE); tabVendedores.push_back(hListViewVendedores); ConfigListView(hListViewVendedores, {L"ID", L"Nome do Vendedor"}, {50, 300});
            CriarCtrl(tabVendas, L"STATIC", L"ID Cliente:", 0, 30, 60, 120, 20, NULL); hEditVenIdCli = CriarEdit(tabVendas, L"", ES_NUMBER, 150, 60, 60, 24, NULL); CriarCtrl(tabVendas, L"STATIC", L"ID Vendedor:", 0, 30, 95, 120, 20, NULL); hEditVenIdVend = CriarEdit(tabVendas, L"", ES_NUMBER, 150, 95, 60, 24, NULL); CriarCtrl(tabVendas, L"STATIC", L"ID Livro:", 0, 30, 130, 120, 20, NULL); hEditVenIdLivro = CriarEdit(tabVendas, L"", ES_NUMBER, 150, 130, 60, 24, NULL); CriarCtrl(tabVendas, L"STATIC", L"Quantidade:", 0, 30, 165, 120, 20, NULL); hEditVenQtd = CriarEdit(tabVendas, L"1", ES_NUMBER, 150, 165, 60, 24, NULL); CriarCtrl(tabVendas, L"STATIC", L"Pagamento:", 0, 30, 200, 120, 20, NULL); hComboMetodo = CriarCtrl(tabVendas, L"COMBOBOX", L"", CBS_DROPDOWNLIST, 150, 200, 120, 100, NULL); SendMessage(hComboMetodo, CB_ADDSTRING, 0, (LPARAM)L"Pix"); SendMessage(hComboMetodo, CB_ADDSTRING, 0, (LPARAM)L"Cartão"); SendMessage(hComboMetodo, CB_ADDSTRING, 0, (LPARAM)L"Berries"); SendMessage(hComboMetodo, CB_SETCURSEL, 0, 0); CriarCtrl(tabVendas, L"BUTTON", L"Finalizar Venda e Baixar Estoque", BS_PUSHBUTTON, 30, 245, 250, 40, (HMENU)ID_BTN_FINALIZAR_VENDA); CriarCtrl(tabVendas, L"STATIC", L"🏆 Relatório BI: Ranking de Desempenho (View)", 0, 320, 60, 400, 20, NULL); hListViewRanking = CreateWindowExW(WS_EX_CLIENTEDGE, WC_LISTVIEWW, L"", WS_CHILD | LVS_REPORT | LVS_SINGLESEL, 320, 85, 670, 375, hwnd, NULL, NULL, NULL); SendMessage(hListViewRanking, WM_SETFONT, (WPARAM)hFont, TRUE); tabVendas.push_back(hListViewRanking); ConfigListView(hListViewRanking, {L"Vendedor", L"Qtd Vendas", L"Faturamento Total"}, {250, 120, 200});
        ...
        case WM_COMMAND: {
            if (HIWORD(wParam) == EN_CHANGE) {
                // Verifica se veio do campo de busca de título
                if (LOWORD(wParam) == ID_EDIT_BUSCA_TITULO) {
                    AtualizarTabelaLivros(); 
                }
            }
            ...
            if (LOWORD(wParam) == ID_BTN_VEND_ATUALIZAR) {
                wchar_t idW[20], n[256];
                GetWindowTextW(hEditVendId, idW, 20);
                GetWindowTextW(hEditNomeVendedor, n, 256);

                if (wcslen(idW) == 0 || wcslen(n) == 0) {
                    MessageBoxW(hwnd, L"Preencha o ID e o novo Nome para atualizar!", L"Aviso", MB_OK | MB_ICONWARNING);
                    return 0;
                }

                wstring msgErro;
                if (db_vendas.atualizarVendedor(_wtoi(idW), n, msgErro)) {
                    MessageBoxW(hwnd, L"Vendedor atualizado com sucesso!", L"Sucesso", MB_OK);
                    SetWindowTextW(hEditNomeVendedor, L"");
                    SetWindowTextW(hEditVendId, L"");
                    AtualizarTabelasGlobais();
                } else {
                    MessageBoxW(hwnd, (L"Erro ou ID não encontrado no banco:\n" + msgErro).c_str(), L"Erro", MB_OK | MB_ICONERROR);
                }
            }
            ...
            if (LOWORD(wParam) == ID_BTN_CLI_BUSCAR) { wchar_t idW[20]; GetWindowTextW(hEditCliBuscaId, idW, 20); if (wcslen(idW) > 0) { int id = _wtoi(idW); wstring n, c, t; bool a; if (db_clientes.buscarPorId(id, n, c, t, a)) { SetWindowTextW(hEditCliNome, n.c_str()); SetWindowTextW(hEditCliCidade, c.c_str()); SetWindowTextW(hEditCliTime, t.c_str()); SendMessage(hCheckCliAnime, BM_SETCHECK, a ? BST_CHECKED : BST_UNCHECKED, 0); ListView_DeleteAllItems(hListViewHistorico); vector<HistoricoItem> hist = db_clientes.buscarHistorico(id, hwnd); if (hist.empty()) { MessageBoxW(hwnd, L"Dados carregados! Mas atenção: Este cliente ainda não fez nenhuma compra (ou houve erro no SQL).", L"Histórico", MB_OK | MB_ICONINFORMATION); } else { for(size_t i=0; i<hist.size(); i++){ LVITEMW lvi={0}; lvi.mask=LVIF_TEXT; lvi.iItem=i; lvi.iSubItem=0; lvi.pszText=(LPWSTR)hist[i].id_venda.c_str(); ListView_InsertItem(hListViewHistorico, &lvi); ListView_SetItemText(hListViewHistorico, i, 1, (LPWSTR)hist[i].livro.c_str()); ListView_SetItemText(hListViewHistorico, i, 2, (LPWSTR)hist[i].qtd.c_str()); ListView_SetItemText(hListViewHistorico, i, 3, (LPWSTR)hist[i].total.c_str()); ListView_SetItemText(hListViewHistorico, i, 4, (LPWSTR)hist[i].metodo.c_str()); } } } else { MessageBoxW(hwnd, L"Ops! Nenhum cliente encontrado com este ID.", L"Erro", MB_OK | MB_ICONERROR); } } }
            if (LOWORD(wParam) == ID_BTN_CLI_ATUALIZAR) {
                wchar_t idW[20]; GetWindowTextW(hEditCliBuscaId, idW, 20);
                if (wcslen(idW) == 0) {
                    MessageBoxW(hwnd, L"Primeiro, busque um cliente pelo ID para editar!", L"Aviso", MB_OK | MB_ICONWARNING);
                    return 0;
                }
                wchar_t n[256], c[100], t[100]; 
                GetWindowTextW(hEditCliNome, n, 256); GetWindowTextW(hEditCliCidade, c, 100); GetWindowTextW(hEditCliTime, t, 100); 
                bool anime = (SendMessage(hCheckCliAnime, BM_GETCHECK, 0, 0) == BST_CHECKED); 
                
                if (wcslen(n) == 0) {
                    MessageBoxW(hwnd, L"O nome do cliente não pode ficar vazio!", L"Erro", MB_OK);
                    return 0;
                }

                wstring msgErro; 
                if (db_clientes.atualizar(_wtoi(idW), n, c, t, anime, msgErro)) { 
                    MessageBoxW(hwnd, L"Dados do cliente atualizados com sucesso!", L"Sucesso", MB_OK); 
                    AtualizarTabelasGlobais(); 
                } else {
                    MessageBoxW(hwnd, (L"Erro ao atualizar:\n" + msgErro).c_str(), L"Erro", MB_OK | MB_ICONERROR);
                }
            }
            if (LOWORD(wParam) == ID_BTN_FINALIZAR_VENDA) { wchar_t c[10], v[10], l[10], q[10], m[50]; GetWindowTextW(hEditVenIdCli, c, 10); GetWindowTextW(hEditVenIdVend, v, 10); GetWindowTextW(hEditVenIdLivro, l, 10); GetWindowTextW(hEditVenQtd, q, 10); SendMessage(hComboMetodo, CB_GETLBTEXT, SendMessage(hComboMetodo, CB_GETCURSEL, 0, 0), (LPARAM)m); if (wcslen(c) == 0 || wcslen(v) == 0 || wcslen(l) == 0 || wcslen(q) == 0) { MessageBoxW(hwnd, L"Preencha todos os IDs e Quantidade!", L"Erro", MB_OK | MB_ICONWARNING); return 0; } int qtdD = _wtoi(q); if (qtdD <= 0) return 0; double precoBase = 0; int estoqueAtual = 0; double totalPerc = 0.0; wstring detalhesDesconto; double total = db_vendas.calcularVenda(_wtoi(c), _wtoi(l), qtdD, precoBase, estoqueAtual, detalhesDesconto, totalPerc); if (precoBase == 0) { MessageBoxW(hwnd, L"Livro não encontrado!", L"Erro", MB_OK | MB_ICONERROR); return 0; } if (estoqueAtual < qtdD) { MessageBoxW(hwnd, (L"Estoque insuficiente!\nTemos apenas " + to_wstring(estoqueAtual)).c_str(), L"Operação Negada", MB_OK | MB_ICONWARNING); return 0; } wchar_t bufP[50], bufT[50], bufPerc[10]; swprintf(bufP, 50, L"%.2f", precoBase * qtdD); swprintf(bufT, 50, L"%.2f", total); swprintf(bufPerc, 10, L"%.0f", totalPerc); wstring msg = L"====== RESUMO DA VENDA ======\n\nPreço Original: R$ " + wstring(bufP) + L" (Qtd: " + wstring(q) + L")\n\nDescontos Aplicados (" + wstring(bufPerc) + L"% no total):\n" + detalhesDesconto + L"\n-------------------------------------------------\nVALOR FINAL A PAGAR: R$ " + wstring(bufT) + L"\n\nConfirma a venda?"; if(MessageBoxW(hwnd, msg.c_str(), L"Confirmar Checkout", MB_YESNO | MB_ICONQUESTION) == IDYES) { wstring msgErroDB; if (db_vendas.finalizarVenda(_wtoi(c), _wtoi(v), _wtoi(l), qtdD, total, w_para_s(m), msgErroDB)) { MessageBoxW(hwnd, L"Venda concluída! O estoque e o ranking foram atualizados.", L"Sucesso", MB_OK | MB_ICONINFORMATION); AtualizarTabelasGlobais(); } else { MessageBoxW(hwnd, (L"Falha ao realizar a venda no Banco de Dados!\n\nDetalhe do MySQL:\n" + msgErroDB).c_str(), L"Erro na Venda", MB_OK | MB_ICONERROR); } } }
            return 0;
        }
